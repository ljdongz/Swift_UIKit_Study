...
class LoginViewModel {
    ...
    var loginButtonStatus: 클래스로감싸진데이터<Bool> = 클래스로감싸진데이터(false)
    ...
    func checkLoginValid() {
        let email = emailString.데이터값
        let password = passwordString.데이터값
        
        if !email.isEmpty && !password.isEmpty {
            loginButtonStatus.데이터값 = true
        } else {
            loginButtonStatus.데이터값 = false
        }
    }
